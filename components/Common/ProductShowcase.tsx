    ...
    <div className="w-full h-full bg-zinc-950 rounded-xl p-5 font-mono text-xs overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-zinc-400 text-[11px]">Live — 1,284 assets monitored</span>
        </div>
        <span className="text-zinc-600 text-[11px]">06 Mar 2026 · 14:32 UTC</span>
      </div>
      ...
      <div className="bg-zinc-900 rounded-lg p-3 border border-zinc-800 mb-3">
        <p className="text-zinc-500 text-[10px] mb-3">Threat Activity — Last 24h</p>
        <div className="flex items-end gap-1 h-14">
          {[3, 7, 4, 9, 5, 12, 8, 6, 3, 15, 11, 7, 4, 8, 6, 10, 13, 9, 5, 7, 4, 11, 8, 6].map(
            (h, i) => (
              <motion.div
                key={i}
                initial={{ scaleY: 0 }}
      ...
      <div className="space-y-1.5">
        {[
          { msg: "Lateral movement detected — 192.168.1.45", sev: "CRITICAL", color: "text-red-400" },
          { msg: "Brute force attempt — admin@corp.com", sev: "HIGH", color: "text-orange-400" },
          { msg: "Unusual outbound traffic — srv-db-02", sev: "MEDIUM", color: "text-yellow-400" },
        ].map((a, i) => (
          <motion.div
            key={i}
          ...
          >
            <span className="text-zinc-400 text-[10px] truncate flex-1">{a.msg}</span>
            <span className={`text-[9px] font-bold ml-2 shrink-0 ${a.color}`}>{a.sev}</span>
          </motion.div>
        ))}
      </div>
    </div>
...
function XDRVisual() {
  const nodes = [
    { label: "Endpoint", x: 15, y: 20, icon: "💻" },
    { label: "Network", x: 75, y: 20, icon: "🌐" },
    { label: "Cloud", x: 15, y: 70, icon: "☁️" },
    { label: "Identity", x: 75, y: 70, icon: "👤" },
  ];
      ...
      <div className="relative flex-1" style={{ height: "220px" }}>
        ...
        {nodes.map((node, i) => (
          ...
          >
            ...
            <span className="text-zinc-500 text-[9px] font-mono">{node.label}</span>
          </div>
        ))}
...
function SIEMVisual() {
  const logs = [
    { time: "14:32:01", src: "fw-edge-01", event: "BLOCKED inbound 45.33.32.156:4444", level: "WARN" },
    { time: "14:32:03", src: "auth-svc", event: "FAILED login user=admin host=10.0.0.5", level: "ERROR" },
    { time: "14:32:07", src: "srv-db-02", event: "Query spike — 3,200 req/s anomaly", level: "WARN" },
    { time: "14:32:11", src: "edr-agent", event: "Process injection attempt PID 4821", level: "CRIT" },
    { time: "14:32:15", src: "cloud-gw", event: "IAM role escalation — arn:aws:iam::921", level: "CRIT" },
    { time: "14:32:19", src: "vpn-gw", event: "AUTH success user=j.doe ip=91.205.4.2", level: "INFO" },
  ];
  ...
  return (
    <div className="w-full h-full bg-zinc-950 rounded-xl p-5 font-mono overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        <p className="text-zinc-400 text-[11px]">Log Stream — 10,284 events/min</p>
        <div className="flex gap-1">
          {["CRIT", "ERROR", "WARN"].map((l) => (
            <span key={l} className={`text-[9px] font-bold px-1.5 py-0.5 rounded bg-zinc-900 ${levelColor[l]}`}>
              {l}
            </span>
          ))}
        </div>
      ...
      <div className="space-y-1.5 overflow-hidden">
        {logs.map((log, i) => (
          ...
          >
            ...
            <span className="text-[#ac1b1b]/70 shrink-0 w-20 truncate">{log.src}</span>
      ...
      >
        <p className="text-zinc-500 text-[10px] mb-2">AI Correlation — New Rule Triggered</p>
...
function AlertsVisual() {
  const alerts = [
    { title: "Ransomware Behavior Detected", asset: "DESKTOP-JK92", time: "just now", sev: "CRITICAL", sevColor: "bg-red-500" },
    { title: "Privileged Account Anomaly", asset: "admin@corp.com", time: "2m ago", sev: "HIGH", sevColor: "bg-orange-500" },
    { title: "Unusual DNS Queries", asset: "srv-web-01", time: "5m ago", sev: "MEDIUM", sevColor: "bg-yellow-500" },
    { title: "Port Scan from Internal Host", asset: "192.168.4.77", time: "12m ago", sev: "MEDIUM", sevColor: "bg-yellow-500" },
  ];
  ...
  return (
    <div className="w-full h-full bg-zinc-950 rounded-xl p-5 font-mono overflow-hidden">
        ...
        {alerts.map((a, i) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-zinc-200 text-[11px] font-semibold truncate">{a.title}</p>
            ...
            <span className={`text-[9px] font-bold shrink-0 ${
              a.sev === "CRITICAL" ? "text-red-400" :
              a.sev === "HIGH" ? "text-orange-400" : "text-yellow-400"
            }`}>
              {a.sev}
            </span>
          </motion.div>
      ...
      >
        {[
          { label: "Sent to Slack", icon: "💬" },
          { label: "PagerDuty", icon: "📟" },
          { label: "Email SOC", icon: "✉️" },
        ].map((i, idx) => (
          <div key={idx} className="flex items-center gap-1.5 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-1.5 text-[10px] text-zinc-500">
            <span>{i.icon}</span>
            <span>{i.label}</span>
          </div>
        ))}
...
function AgentsVisual() {
  ...
  return (
    <div className="w-full h-full bg-zinc-950 rounded-xl p-5 font-mono overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        <p className="text-zinc-400 text-[11px]">Agent Fleet — 1,284 deployed</p>
      ...
      <div className="space-y-2 mb-4">
        {agents.map((a, i) => (
          ...
          >
            ...
            <span className="text-zinc-300 text-[11px] flex-1 font-semibold">{a.host}</span>
            <span className="text-zinc-600 text-[10px] w-28 truncate">{a.os}</span>
            <span className="text-zinc-500 text-[10px] w-8 text-right">{a.cpu}</span>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-lg p-2.5 text-center">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-zinc-400 text-lg max-w-xl mx-auto">
            Active XDR protection from modern threats. A comprehensive SIEM solution.
            Built for the speed of today&apos;s adversaries.
          </p>
        </motion.div>
        ...
        >
          {[
            { icon: Eye, label: "Full Visibility", sub: "Every asset, every event" },
            { icon: Zap, label: "0.3s Response", sub: "Automated threat disruption" },
            { icon: Lock, label: "Zero Trust Ready", sub: "Identity-first architecture" },
            { icon: Network, label: "Any Environment", sub: "Cloud, on-prem, hybrid" },
          ].map((item, i) => {
