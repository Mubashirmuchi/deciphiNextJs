...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    if (latest < previous) {
      ...
      // Scrolling DOWN past 10px — hide navbar
      setVisible(false);
    }
      ...
      <AnimatePresence>
        {visible && (
          <motion.div
            ...
            className="w-full bg-white shadow-md lg:hidden"
          >
            {/* Full width bg, but inner content constrained to max-w-7xl */}
                ...
                (menu, index) => (
                  ...
                  >
                    <Link
                      ...
                      onClick={() => setIsOpen(false)}
                    >
                      <Button className="w-full text-left px-4 py-4 hover:bg-white/10 rounded-lg text-white/70 hover:text-white/90">
                        {menu.name}
                      </Button>
                    </Link>
                  </motion.div>
